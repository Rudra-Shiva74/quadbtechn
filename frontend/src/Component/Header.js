...
const Header = (props) => {
  ...
  const fetchProductsCount = async () => {
    try {
      const resp = await axios.post(`${props.envvariable.apiUrl}checktocart`, { email: isUserLogin() && isUserLogin().email }, {
        headers: {
  ...
  return (
    <header className="header">
      ...
      <div className="icons">
        {isUserLogin() ? <IoIosLogIn size={20} style={{ cursor: "pointer" }} onClick={() => props.signout(false)} /> : <Link to={'/signin'}> <HiOutlineLogin size={20} style={{ cursor: "pointer" }} /></Link>}
        <Link to={'/user/cart'}><div className="cart-icon">
          <BsCart3 size={20} style={{ cursor: "pointer" }} />
          {isUserLogin() ? <span className="badge">{count}</span> : ''}
        </div>
        </Link>
        <Link to={'/user/dashboard'}><FaRegUserCircle size={20} style={{ cursor: "pointer" }} /></Link>
      </div>
    </header>
  );
};
